...
目标页面: https://blog.google/products/gemini/
页面特点: Google Blog，需要 JavaScript 渲染
"""
...
class GeminiExtractor(BaseExtractor):
    ...
    def get_extraction_schema(self) -> dict:
        ...
        return {
            ...
            "fields": [
                {
                    "name": "title",
                    "selector": "h2, h3, .title, span[class*='headline']",
                    "type": "text",
                },
                {
                    "name": "url",
                ...
                {
                    "name": "summary",
                    "selector": "p, .description, .excerpt, .summary",
                    "type": "text",
                },
            ],
        }
    ...
    def parse_result(
        ...
        for article in articles:
            ...
            # 过滤非 Gemini 相关链接
            if "/products/gemini/" not in url and "gemini" not in url.lower():
                continue
